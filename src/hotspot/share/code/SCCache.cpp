...
bool SCConfig::verify(const char* cache_path) const {
  ...
  if (_contendedPaddingWidth != (uint)ContendedPaddingWidth) {
    log_warning(scc, init)("Disable Startup Code Cache: '%s' was created with ContendedPaddingWidth = %d vs current %d", cache_path, _contendedPaddingWidth, (int)ContendedPaddingWidth);
    return false;
  }
